...
import { registerUser, verifyUserAccount } from '../auth';
...
describe('testing auth user registration', () => {
	...
	it('tests user registration succeeds', () => {
		...
		const expectedAction = [{ type: actionTypes.SIGNUP_USER_STARTED },
			{ type: actionTypes.SIGNUP_USER_SUCCEEDED, data: {} }];
...
describe('testing user account verification', () => {
	...
	it('tests user verification succeeds', () => {
		...
		const expectedAction = [{ type: actionTypes.VERIFY_USER_ACCOUNT_STARTED}, {
